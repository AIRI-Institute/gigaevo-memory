    ...
    def load_agents() -> Tuple[List[List], List, str, str]:
        ...
        try:
            ...
            for agent in items:
                ...
                meta = agent.get("meta", {})
                name = pick_display_name(meta, agent.get("display_name"))
                ...
                fav = format_favourite(agent.get("favourite"))
                last_run = format_last_run(agent.get("last_run_at"))
                table_data.append(
    ...
    with gr.Row():
        ...
        with gr.Column(scale=1):
            ...
            agents_list = gr.Dataframe(
                ...
                column_count=7,
                interactive=False,
                label="Agents (favourite ⭐ first, recent runs surfaced)",
            )
