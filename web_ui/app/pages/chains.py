...
def chains_tab(client):
    """Create the chains tab with list, editor, and version management."""
    ...
    def load_chains() -> Tuple[List[List], List, str, str]:
        """Load chains list. Returns (data_for_table, raw_data, status_message, last_update)."""
        try:
            result = client.get_chains(limit=100)
            if not result:
                return create_refresh_result([], [], "✅ No chains found")
            ...
            return create_refresh_result(
                table_data, raw_data, f"✅ Loaded {len(table_data)} chains"
            )
        except Exception as e:
            return create_refresh_result([], [], f"❌ Error loading chains: {str(e)}")
    ...
    def load_chain(
        ...
        if not entity_id:
            return "", "", "", "⚠️ No chain ID provided"

        try:
            chain = client.get_chain(entity_id, channel=channel)
            entity_id_resp, name, content = extract_entity_fields(chain)
            return (
                entity_id_resp,
                name,
                content,
                f"✅ Loaded chain: {name} (channel: {channel})",
            )
        except Exception as e:
            return "", "", "", f"❌ Error loading chain: {str(e)}"
    ...
    def save_chain(
        ...
        """Save chain. Returns (entity_id, status_message, refresh_trigger)."""
        try:
    ...
    def delete_chain(entity_id: str) -> Tuple[str, str]:
        """Delete a chain. Returns (status_message, refresh_trigger)."""
        if not entity_id:
            return "⚠️ No chain ID provided", ""

        try:
            client.delete_chain(entity_id)
            return f"✅ Deleted chain: {entity_id}", "refresh"
        except Exception as e:
    ...
    def load_versions(entity_id: str) -> Tuple[List[List], List, str]:
        """Load versions for a chain."""
        if not entity_id:
            return [], [], "⚠️ Select a chain first"

        table_data, versions, msg = load_versions_list(client, entity_id, "chain", 50)
    ...
    def load_version_content(entity_id: str, version_id: str) -> str:
        ...
        content, msg = load_version_detail(client, entity_id, version_id, "chain")
        return content
    ...
    def pin_version(entity_id: str, channel: str, version_id: str) -> str:
        ...
        return pin_channel_version(client, entity_id, channel, version_id, "chain")
    ...
    def promote_channels(entity_id: str, from_channel: str, to_channel: str) -> str:
        ...
        return promote_channel(client, entity_id, from_channel, to_channel, "chain")
    ...
    with gr.Row():
        # Left: Editor
        with gr.Column(scale=1):
            gr.Markdown("### ✏️ Chain Editor")

            chain_id_display = gr.Textbox(
                label="Chain ID",
                interactive=False,
                placeholder="Auto-generated for new chains",
            )

            chain_name = gr.Textbox(label="Name", placeholder="Enter chain name...")

            with gr.Row():
                chain_channel = gr.Dropdown(
                    label="Channel", choices=["latest", "stable"], value="latest"
                )
                chain_tags = gr.Textbox(
                    label="Tags (comma separated)", placeholder="tag1, tag2"
                )
                chain_author = gr.Textbox(label="Author", placeholder="Author name")

            chain_content = gr.Code(label="Content (JSON)", language="json", lines=20)
    ...
    load_btn.click(
        fn=load_chain,
        inputs=[selected_entity_id, chain_channel],
        outputs=[chain_id_display, chain_name, chain_content, status_editor],
    )
    ...
    save_btn.click(
        ...
        outputs=[chain_id_display, status_editor, refresh_trigger],
    )

    # Delete entity
    delete_btn.click(
        fn=delete_chain,
        inputs=[chain_id_display],
    ...
    def refresh_all(refresh_trig):
        table_data, raw_data, status, last_update = load_chains()
    ...
    gr.on(
        ...
        outputs=[chains_list, list_data_state, status_list, last_update_display],
    )
    ...
    view_version_btn.click(
        ...
        inputs=[chain_id_display, selected_version_id],
    ...
    diff_btn.click(
        fn=show_diff,
        inputs=[chain_id_display, diff_from, diff_to],
        outputs=[diff_output],
    )
    ...
    revert_btn.click(
        ...
        inputs=[chain_id_display, revert_to],
        outputs=[chain_id_display, revert_status],
    )

    # Pin channel
    pin_btn.click(
        fn=lambda eid, ch, vid: pin_version(eid, ch, vid),
        inputs=[chain_id_display, pin_channel, pin_version],
    ...
    promote_btn.click(
        ...
        inputs=[chain_id_display, promote_from, promote_to],
        outputs=[status_version_actions],
    )

    # Initial load trigger - hidden component that changes on app load
