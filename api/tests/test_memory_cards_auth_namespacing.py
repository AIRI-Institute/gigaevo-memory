"""Tests for auth + namespace defaulting on the memory_cards router
(P1 §3 follow-up — iteration #33). Closes out the writes-side
rollout that started with the bulk router in iter #29."""
...
def _card_body(*, namespace: str | None = None) -> dict:
    body: dict = {
        "meta": {"name": "incident-2026-04-tax-deadline"},
        "content": {
            "version": "1.0",
            "summary": "Tax filing deadline missed for Q1 2026.",
            "lessons": ["Set calendar reminder 2 weeks earlier."],
        },
        "channel": "latest",
    }
...
def _capture_create():
    ...
    async def _spy(self, **kw):
        ...
        entity.display_name = "incident-2026-04-tax-deadline"
...
class TestMemoryCardsCreateOptIn:
    ...
    def test_no_key_explicit_namespace_respected(self, client, _capture_create):
        _stub_db_for_auth(None)
        resp = client.post(
            "/v1/memory-cards", json=_card_body(namespace="shared")
        )
        assert resp.status_code == 201
        assert _capture_create["namespace"] == "shared"


class TestMemoryCardsCreateAuthenticated:
    ...
    def test_valid_key_explicit_namespace_wins(self, client, _capture_create):
        ...
        resp = client.post(
            "/v1/memory-cards",
            headers={"X-API-Key": plaintext},
            json=_card_body(namespace="finance-team"),
        )
        assert resp.status_code == 201
...
class TestMemoryCardsCreateStrict:
