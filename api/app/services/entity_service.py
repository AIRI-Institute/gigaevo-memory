...
import jsonpatch
from sqlalchemy import ARRAY, Text, and_, cast, func, or_, select, text
...
class EntityService:
    ...
    async def list_entities(
        ...
        *,
        ...
        if tags:
            # ``?&`` is the ``jsonb ?& text[]`` operator — bind the tokens as a
            # text[] (cast), NOT JSONB. Letting SQLAlchemy infer the bind type
            # from the JSONB ``tags`` column renders ``?& $n::JSONB`` and
            # Postgres errors "operator does not exist: jsonb ?& jsonb",
            # 500-ing every tag filter.
            stmt = stmt.where(Entity.tags.op("?&")(cast(list(tags), ARRAY(Text))))
        if q:
            like = f"%{q}%"
            stmt = stmt.where(
                or_(
