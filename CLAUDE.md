...
GigaEvo Memory is a persistent memory subsystem for CARL artifacts (steps, chains, agents, agent skills, memory cards). It consists of three Python packages managed as a `uv` workspace:
...
- `client/python/` (`gigaevo-memory`) — Public Python SDK distributed on PyPI. Package was renamed `gigaevo_memory` → `gigaevo_client` in 0.3.0; the old import path is a shim that emits a one-time `DeprecationWarning`.

Python 3.12+. Workspace root `pyproject.toml` aggregates all three members.
...
### Testing
- `make test` — unit tests for API + client (no Docker).
- `make test-api-unit` — fast API unit subset (explicit file list in the Makefile target — when adding new unit tests that should run without Docker, add them there).
...
- `make client-test` — client unit tests.
- `make test-roundtrip` — golden round-trip CARL compatibility tests under `client/python/tests/test_roundtrip.py`.
- Single test: `cd api && uv run pytest tests/test_entity_service.py::test_name -v` (or analogous in `client/python`).

### Lint / format
- `make lint` — `ruff check` over `api/app/` and the client.
- `make fmt` — `ruff format` (line length 120, set at workspace root).
- `make client-lint` adds `mypy` for the client; mypy is **not** enforced on the API.

### Client release
- `make client-install` / `client-build` / `client-publish` / `client-publish-test`.
- `make client-version v=X.Y.Z` — bumps `__version__` in `client/python/src/gigaevo_memory/__init__.py` (only sed line; the canonical `gigaevo_client/__init__.py` also carries `__version__` — keep them in sync if you touch it manually).
...
### Client config layering
`GigaEvoConfig.load()` composes (lowest to highest precedence): class defaults → `~/.config/gigaevo/config.toml` → env vars (`GIGAEVO_MEMORY_URL`, `GIGAEVO_PLATFORM_URL`, `GIGAEVO_API_KEY`). New client features should accept a `GigaEvoConfig` instance, not raw URL/key strings.

### Package rename
Imports inside the client package use `gigaevo_client`. Tests still use `gigaevo_memory` in places (with the deprecation warning silenced in `client/python/pyproject.toml`'s `filterwarnings`). New code should use `gigaevo_client`. `test_no_legacy_gigaevo_memory_imports.py` (API tests) enforces that the **server** never imports the legacy shim.
