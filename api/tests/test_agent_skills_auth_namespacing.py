"""Tests for auth + namespace defaulting on the agent_skills router
(P1 §3 follow-up — iteration #32). Same shape as the agents suite so
the rollout reads as a copy-paste diff."""
...
def _skill_body(*, namespace: str | None = None) -> dict:
    body: dict = {
...
def _capture_create():
    ...
    async def _spy(self, **kw):
        ...
        entity.display_name = "pdf-extract"
...
class TestAgentSkillsCreateOptIn:
    ...
    def test_no_key_explicit_namespace_respected(self, client, _capture_create):
        _stub_db_for_auth(None)
        resp = client.post(
            "/v1/agent-skills", json=_skill_body(namespace="shared")
        )
        assert resp.status_code == 201
        assert _capture_create["namespace"] == "shared"


class TestAgentSkillsCreateAuthenticated:
    ...
    def test_valid_key_explicit_namespace_wins(self, client, _capture_create):
        ...
        resp = client.post(
            "/v1/agent-skills",
            headers={"X-API-Key": plaintext},
            json=_skill_body(namespace="finance-team"),
        )
        assert resp.status_code == 201
...
class TestAgentSkillsCreateStrict:
