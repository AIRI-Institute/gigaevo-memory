...
class TestAgentSkillsRouter:
    ...
    def test_router_mounted_on_app(self):
        ...
        # Use the OpenAPI path table: newer FastAPI wraps included routers in
        # `_IncludedRouter` objects that have no `.path`, so iterating
        # `app.routes` directly breaks.
        full_paths = set(app.openapi()["paths"])
